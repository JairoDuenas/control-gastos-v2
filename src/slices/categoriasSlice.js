...
const save = (list) => {
  try { localStorage.setItem('gastos_categorias', JSON.stringify(list)) } catch {}
}

const nextId = (list) => list.length ? Math.max(...list.map(c => c.id)) + 1 : 1
...
export const { addCategoria, editCategoria, deleteCategoria } = categoriasSlice.actions
