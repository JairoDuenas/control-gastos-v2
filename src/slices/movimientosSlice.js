...
const save = (list) => {
  try { localStorage.setItem('gastos_movimientos', JSON.stringify(list)) } catch {}
}
...
const calcTotales = (list) => {
  ...
  const pagados  = list.filter(m => m.estado === 1).reduce((a, m) => a + m.monto, 0)
  const pendientes = list.filter(m => m.estado === 0).reduce((a, m) => a + m.monto, 0)
...
const movimientosSlice = createSlice({
  ...
  initialState: {
    list: load(),
    filtered: load(),
    filtros: { mes: new Date().getMonth() + 1, anio: new Date().getFullYear(), categoriaId: null, texto: '' },
    ...calcTotales(load()),
  },
  reducers: {
    addMovimiento: (state, action) => {
      const item = { ...action.payload, id: nextId(state.list), fecha: action.payload.fecha || new Date().toISOString() }
    ...
    editMovimiento: (state, action) => {
      ...
      if (idx >= 0) { state.list[idx] = action.payload; save(state.list) }
      movimientosSlice.caseReducers._applyFilter(state)
    },
    ...
    _applyFilter: (state) => {
      ...
      if (mes)        result = result.filter(m => new Date(m.fecha).getMonth() + 1 === mes)
      if (anio)       result = result.filter(m => new Date(m.fecha).getFullYear() === anio)
      if (categoriaId) result = result.filter(m => m.categoriaId === categoriaId)
      if (texto)      result = result.filter(m => m.descripcion.toLowerCase().includes(texto.toLowerCase()))
